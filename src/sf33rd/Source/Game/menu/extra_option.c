...
static void adjust_extra_option_value(u16 input) {
    if (input == SWK_WEST) {
...
static void wrap_regular_extra_option(u16 input) {
    if (input == SWK_WEST && save_w[1].extra_option.contents[Menu_Page_Buff][Menu_Cursor_Y[0]] < 0) {
        save_w[1].extra_option.contents[Menu_Page_Buff][Menu_Cursor_Y[0]] =
            Ex_Menu_Max_Data[Menu_Page][Menu_Cursor_Y[0]];
    }
    if (input == SWK_EAST && save_w[1].extra_option.contents[Menu_Page_Buff][Menu_Cursor_Y[0]] >
                       Ex_Menu_Max_Data[Menu_Page][Menu_Cursor_Y[0]]) {
        save_w[1].extra_option.contents[Menu_Page_Buff][Menu_Cursor_Y[0]] = 0;
    }
}

static void finish_final_extra_option(u16 input, u8 previous_value) {
    if (input == SWK_WEST && save_w[1].extra_option.contents[Menu_Page_Buff][Menu_Cursor_Y[0]] < 0) {
        save_w[1].extra_option.contents[Menu_Page_Buff][Menu_Cursor_Y[0]] = 0;
        IO_Result = 0x80;
        return;
    }
    if (input == SWK_EAST && save_w[1].extra_option.contents[Menu_Page_Buff][Menu_Cursor_Y[0]] > 2) {
    ...
    if (save_w[1].extra_option.contents[Menu_Page_Buff][Menu_Cursor_Y[0]] != previous_value) {
        Message_Data->order = 1;
...
static void move_extra_option(u16 input, u8 previous_value) {
    if (is_extra_option_page_item()) {
        SE_dir_cursor_move();
    }
    adjust_extra_option_value(input);
    if (Menu_Cursor_Y[0] != Menu_Max) {
        wrap_regular_extra_option(input);
        return;
    }
    finish_final_extra_option(input, previous_value);
}
