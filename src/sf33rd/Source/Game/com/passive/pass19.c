...
static void pattern_em_term_branch_unit_area(PLW* wk, const EM_Term_Params* p, const Branch_Menu_Args* p_b) {
    switch (CP_Index[wk->wu.id][0]) {
    case 0:
        EM_Term(wk, p);
        break;
...
void Passive19_0048(PLW* wk) {
    pattern_normal_attack_2(wk, 0x12, 8, 0x22);
}
...
void Passive19_0079(PLW* wk) {
    pattern_jump_attack_term_normal_attack(wk, &(Jump_Term_Args){-0x7F60, -0x7FB0, 0xB, 0x400, 0, -0x7FA0, -1, 0x20});
}
...
void Passive19_0139(PLW* wk) {
    pattern_jump_attack(wk, &(Jump_Attack_Args){0xC, 0xD, 0x400, 2});
}
