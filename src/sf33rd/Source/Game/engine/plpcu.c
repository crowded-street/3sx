...
/* The one caught pattern the port adds and CPS3 does not have. */
static s32 is_port_only_caught_pattern(const PLW* wk) {
    return !ArcadeBalance_IsEnabled() && wk->wu.now_koc == 3 && wk->wu.char_index == 60;
}
...
static void advance_caught_animation(PLW* wk) {
    if (is_port_only_caught_pattern(wk)) {
