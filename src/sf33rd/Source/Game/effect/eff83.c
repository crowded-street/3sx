/**
 * @file eff83.c
 * TODO: identify what this effect does
 */

#include "sf33rd/Source/Game/effect/eff83.h"
#include "bin2obj/char_table.h"
#include "common.h"
#include "sf33rd/Source/Game/effect/effect.h"
#include "sf33rd/Source/Game/engine/caldir.h"
#include "sf33rd/Source/Game/engine/charset.h"
#include "sf33rd/Source/Game/engine/slowf.h"
#include "sf33rd/Source/Game/engine/workuser.h"
#include "sf33rd/Source/Game/rendering/aboutspr.h"
#include "sf33rd/Source/Game/rendering/texcash.h"
#include "sf33rd/Source/Game/sound/se.h"
#include "sf33rd/Source/Game/stage/bg.h"
#include "sf33rd/Source/Game/stage/bg_sub.h"
#include "sf33rd/Source/Game/stage/ta_sub.h"

static s32 game_is_active(void) {
    return !EXE_flag && !Game_pause;
}


static s32 animation_can_advance(void) {
    return !EXE_flag && !Game_pause;
}


/* Travel until close enough to the master, then sound the effect and start the
 * depth move. */
static void e83_approach(WORK_Other* ewk, const WORK* oya_ptr) {
    s16 work;

    if (game_is_active()) {
        ewk->wu.old_rno[0]--;
        add_x_sub(&ewk->wu);
        add_y_sub(&ewk->wu);
        work = ewk->wu.xyz[0].disp.pos - oya_ptr->xyz[0].disp.pos;

        if (work < 0) {
            work = -work;
        }

        if (work < 113) {
            ewk->wu.routine_no[0]++;
            Sound_SE((ewk->master_id * 768) + 350);
            char_move_z(&ewk->wu);
        }
    }
}

/* Keep closing while the timer runs, then signal the master and switch to the
 * finishing animation. */
static void e83_close_in(WORK_Other* ewk, WORK* oya_ptr) {
    if (animation_can_advance()) {
        char_move(&ewk->wu);
        add_x_sub(&ewk->wu);
        add_y_sub(&ewk->wu);
        ewk->wu.old_rno[0]--;

        if (ewk->wu.old_rno[0] <= 0) {
            ewk->wu.routine_no[0]++;
            oya_ptr->cmwk[1] = 9;
            set_char_move_init(&ewk->wu, 0, 35);
        }
    }
}

void effect_83_move(WORK_Other* ewk) {
    WORK* oya_ptr = (WORK*)ewk->my_master;

    switch (ewk->wu.routine_no[0]) {
    case 0:
        ewk->wu.routine_no[0]++;
        ewk->wu.disp_flag = 1;
        setup_shadow_of_the_Effy(&ewk->wu);
        set_char_move_init(&ewk->wu, 0, ewk->wu.char_index);
        ewk->wu.old_rno[0] = 50;
        cal_all_speed_data(&ewk->wu, ewk->wu.old_rno[0], oya_ptr->xyz[0].disp.pos, ewk->wu.xyz[1].disp.pos, 2, 2);
        break;

    case 1:
        e83_approach(ewk, oya_ptr);
        suzi_sync_pos_set(ewk);
        sort_push_request(&ewk->wu);
        break;

    case 2:
        e83_close_in(ewk, oya_ptr);
        suzi_sync_pos_set(ewk);
        sort_push_request(&ewk->wu);
        break;

    case 3:
        if (!EXE_flag && !Game_pause) {
            char_move(&ewk->wu);
        }

        suzi_sync_pos_set(ewk);
        sort_push_request(&ewk->wu);
        break;
    }
}

/* Facing right: come in from the right, from beyond the master when it is
 * already past the stage centre, otherwise from the edge. */
static void e83_enter_facing_right(WORK_Other* ewk, const WORK* wk) {
    if (wk->xyz[0].disp.pos > bg_w.bgw[1].wxy[0].disp.pos) {
        ewk->wu.xyz[0].disp.pos = wk->xyz[0].disp.pos + 256;
    } else {
        ewk->wu.xyz[0].disp.pos = bg_w.bgw[1].wxy[0].disp.pos + (bg_w.pos_offset + 32);
    }
}

/* Facing left: the mirror of the above. */
static void e83_enter_facing_left(WORK_Other* ewk, const WORK* wk) {
    if (wk->xyz[0].disp.pos < bg_w.bgw[1].wxy[0].disp.pos) {
        ewk->wu.xyz[0].disp.pos = wk->xyz[0].disp.pos - 256;
    } else {
        ewk->wu.xyz[0].disp.pos = bg_w.bgw[1].wxy[0].disp.pos - (bg_w.pos_offset + 32);
    }
}

s32 effect_83_init(WORK* wk) {
    WORK_Other* ewk;
    s16 ix;

    if ((ix = pull_effect_work(4)) == -1) {
        return -1;
    }

    ewk = (WORK_Other*)frw[ix];
    ewk->wu.be_flag = 1;
    ewk->wu.id = 83;
    ewk->master_id = wk->id;
    ewk->wu.work_id = 16;
    ewk->wu.cgromtype = 1;
    ewk->wu.my_col_mode = wk->my_col_mode;
    ewk->wu.my_col_code = wk->my_col_code + 6;
    ewk->wu.my_family = wk->my_family;
    ewk->my_master = wk;
    ewk->wu.rl_flag = wk->rl_flag;

    if (wk->rl_flag) {
        e83_enter_facing_right(ewk, wk);
        ewk->wu.old_rno[2] = wk->xyz[0].disp.pos + 56;
    } else {
        e83_enter_facing_left(ewk, wk);
        ewk->wu.old_rno[2] = wk->xyz[0].disp.pos - 56;
    }

    ewk->wu.xyz[1].disp.pos = wk->xyz[1].disp.pos - 12;
    ewk->wu.my_priority = wk->my_priority - 12;
    ewk->wu.position_z = ewk->wu.my_priority - 12;
    *ewk->wu.char_table = _etc2_char_table;
    ewk->wu.char_index = 34;
    ewk->wu.sync_suzi = 0;
    suzi_offset_set(ewk);
    ewk->wu.my_mts = 14;
    ewk->wu.my_trans_mode = get_my_trans_mode(ewk->wu.my_mts);
    return 0;
}
