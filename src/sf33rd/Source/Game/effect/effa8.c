...
const char* Letter_Data_A8DC[69] = { "-PLAY(  )",
                                     " STOP(  ) BUTTON-",
...
const char* Letter_Data_A8AC[69] = { "-PLAY(  )",
                                     " STOP(  ) BUTTON-",
...
const char** Letter_Data_A8[2] = { Letter_Data_A8DC, Letter_Data_A8AC };
    ...
    case 0:
        ...
        if (ewk->wu.disp_flag) {
            if (Order_Dir[ewk->wu.dir_old]) {
                dispButtonImage(&(ButtonImage){ -87, 25, 68, 22, 17, 0, 4, 0 });
            } else {
                dispButtonImage(&(ButtonImage){ -23, 0, 68, 22, 17, 0, 5, 0 });
            }
        }
