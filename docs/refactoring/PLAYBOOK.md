...
| `pls03.c` | 8.38 | *was 8.08.* Recipe T on the table pair every `waza_select` arm chooses from and on the meoshi lever scan, then Recipe E on the leap input test and the catch target. Bumpy Road is clear; what is left is `decode_wst_data`'s twelve encodings and `waza_select`'s eleven case labels, neither reducible without renumbering |
...
| `pls03_super_arts.c` | 9.61 | *was 7.61.* Recipe C on the full-gauge guards and on the EX strength launch, Recipe D on the super-art launch tail, then **Recipe F** on the strength scan both EX paths run - which cleared the pair, both Bumpy Roads and the mean at once. The direct-cancel side was then taken the same way and reverted: each step closed one twin and exposed another, landing at 8.81 against the 9.61 already in hand |
