...
| R03 | 1.82 | 1.82 | **7.60** | Plateau. Split into `pls03_super_arts.c` (7.45). What stops both files is in `PLAYBOOK.md`: `decode_wst_data`'s twelve encodings, `waza_select`'s five table pairs, and the grounded/airborne super-art halves that differ in two values each. |
| R04 | **1.97** | 2.25 | **2.25** | wave 1 of 3 done (PR #3) |
| R05 | 2.31 | 2.31 | **9.39** | Plateau. Split into `cmd_main_checks.c` (7.12). `check_10` and `check_12` were finally merged - one split, then the other called its helper - but `check_23` and the `check_18`/`check_19` pair remain near-miss siblings. |
| R06 | 2.57 | 2.57 | - | not started |
| R07 | 2.58 | 2.58 | - | not started |
| R08 | 2.64 | 2.64 | - | not started |
| R09 | 2.74 | 2.74 | **10.00** | Done. Split into `pls00_normal_states.c` (7.07) and `pls00_damage_states.c` (9.09); the original is at 10.00. The two state files plateau on near-miss `nm_*` siblings. |
...
| R15 | 3.68 | 3.68 | **9.53** | Plateau. Split into `plpdm_states.c` (9.38). Both are down to Overall Code Complexity only; every further arm extraction makes a twin of an existing `begin_damage_*`. |
| R16 | 3.75 | 3.75 | - | not started |
| R17 | 3.86 | 3.86 | - | not started |
| R18 | 3.92 | 3.92 | **9.68** | Plateau. Split twice, into `charset_commands.c` (10.00) and `charset_position.c` (9.09). What stops the original is `set_char_move_init2`'s five arguments: Recipe A would clear it, but one of its 59 call sites is in `plpat00.c`, which this branch may not touch. |
